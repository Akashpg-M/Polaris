	...
	for _, c := range candidates {
		...
		if dist <= radiusKm {
			...
			results = append(results, MatchResult{
				NodeID:     strings.TrimPrefix(c.ID, tenantID+":"),
...
// ApplyEnvelope independently protects in-memory state from duplicate, stale,
// and retired-boot telemetry. Redis repeats this decision atomically.
func (e *Engine) ApplyEnvelope(envelope *events.TelemetryEnvelope) Classification {
	...
	if apply {
		if _, old := shard.nodes[key]; old {
			e.qt.Remove(key)
		}
		payload := proto.Clone(envelope.Payload).(*pb.SpatialObject)
		shard.nodes[key] = payload
		shard.versions[key] = current
		e.qt.Insert(quadtree.Point{Lat: payload.Lat, Lon: payload.Lon, ID: key, Class: uint16(payload.Type), TenantID: payload.TenantId})
	}
	shard.mu.Unlock()
